import { precoAVista, precoParcelado12x } from "@partials/angular-moderno/sections/prices/prices";
...
test.describe("Cursos page - Angular Moderno", () => {
  test.describe("With UTM params", () => {
    test.beforeEach(async ({ page }) => {
      ...
      await page.goto("/cursos/angular-moderno?" + url.toString());
    });

    test("should scroll to prices section when clicking the banner button", async ({ page }) => {
      await page.getByText("Quero Aprender Angular Moderno").first().click();
    ...
    test.describe("Purchase flow", () => {
      ...
      async function testPurchaseFlow({
        ...
        expect(requestBody.mailingListId).toBe(mailingListId.angularModerno);
  ...
  test.describe("Without UTM params", () => {
    test.beforeEach(async ({ page }) => {
      await page.goto("/cursos/angular-moderno");
    });
